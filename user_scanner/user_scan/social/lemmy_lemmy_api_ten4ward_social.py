...
def validate_lemmy_lemmy_api_ten4ward_social(user: str) -> Result:
    encoded_user = urllib.parse.quote(user)
    url = "https://lemmy-api.ten4ward.social/api/v3/user"
    show_url = f"https://lemmy-api.ten4ward.social/u/{encoded_user}"
    headers = {
    ...
    def process(response) -> Result:
        if response.status_code in (400, 404):
            try:
                ...
                if "couldnt_find" in err or "not_found" in err or "not found" in err:
